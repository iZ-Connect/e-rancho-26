...
import { Printer, ShieldCheck, User, Key, Edit3, Save, X, Ban, Lock } from 'lucide-react';
...
const MyID: React.FC<MyIDProps> = ({ user, viewer, onUpdateMilitar, onUpdatePin }) => {
  ...
  // LÓGICA ROBUSTA PARA DADOS
  const postoExibicao = user.posto || user.posto_grad || user["Posto"] || "Posto/Grad";
  const nomeCompletoExibicao = user.nome || user["Nome"] || user.nome_completo || user["Nome Completo"] || "";
    ...
    <div className="max-w-md mx-auto space-y-8 py-4 animate-in fade-in duration-500 pb-20">
      ...
      <div className={`p-8 rounded-[2rem] border border-white/10 shadow-2xl relative overflow-hidden flex flex-col items-center text-center transition-all duration-500 ${isInactive
        ...
        <div className="flex flex-col items-center mb-6">
          <span className={`text-[15px] font-black  tracking-[0.5em] mb-1 ${isInactive ? 'text-red-300' : 'text-white'}`}>
            e-Rancho
          </span>
        ...
        {isEditing ? (
          <div className="w-full space-y-4 mb-6 text-left">
            ...
            <div>
              ...
              <input
                className="w-full bg-white/5 border border-white/10 rounded-xl h-10 px-4 text-white text-sm focus:bg-black/20"
                value={editData.nome || editData["Nome"] || editData.nome_completo || ''}
            ...
            {/* NOVO: Editar E-mail */}
            <div>
              <label className="text-[9px] font-bold text-slate-400 uppercase ml-1">E-mail de Login</label>
              <input
                type="email"
                className="w-full bg-white/5 border border-white/10 rounded-xl h-10 px-4 text-white text-sm focus:bg-black/20"
            ...
            <div>
              ...
              <input
                className="w-full bg-white/5 border border-white/10 rounded-xl h-10 px-4 text-white text-sm focus:bg-black/20"
                value={editData.posto || editData["Posto"] || editData.posto_grad}
                onChange={e => setEditData({
                  ...editData,
                  posto: e.target.value,
                  posto_grad: e.target.value,
                  "Posto": e.target.value
                })}
              />
            </div>
            ...
            {isAdmin && (
              <div>
                ...
                <select
                  ...
                  value={editData.ativo === false ? 'false' : 'true'}
          ...
          <div className="space-y-1 mb-8">
            <h2 className="text-2xl font-black text-white uppercase leading-tight">{user.nome_guerra || "Militar"}</h2>

            {/* Nome Completo */}
            <p className="text-xs font-bold text-white/80 uppercase tracking-wide leading-tight mt-1">
              {nomeCompletoExibicao}
            </p>
        ...
        {!isEditing && (
          <div className="w-full grid grid-cols-2 gap-4">
            ...
            <div className="p-3 bg-white/5 rounded-xl border border-white/5">
              <p className="text-[9px] font-bold text-slate-500 uppercase">Posto/Grad</p>
              <p className="text-xs font-bold text-white truncate uppercase">
                {postoExibicao}
              </p>
            </div>
